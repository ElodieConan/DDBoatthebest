...
class GpsIO:
    ...
    def full_cold_restart(self):
        pmtk = "PMTK104"
        self.send_mtk_cmd (pmtk)
        rcv = self.get_mtk_status ()

    def set_sbas_enabled(self, enable):
        # sbas enabled if enable is true
        if enable:
            pmtk = "PMTK313,1"
        else:
            pmtk = "PMTK313,0"
    ...
    def set_dgps_mode(self,dgps=0):
        # dgps = 2 for  WAAS (EGNOS ?)
        pmtk = "PMTK301,%1d"%dgps
        self.send_mtk_cmd (pmtk)
        rcv = self.get_mtk_status ()
        return rcv

    def query_dgps_mode(self):
        # dgps = 2 for  WAAS (EGNOS ?)
        pmtk = "PMTK401"
        self.send_mtk_cmd (pmtk)
        rcv = self.get_mtk_status ()
    ...
    def set_nmea_update_rate (self,rate):
        pmtk = "PMTK220,%d"%(rate)
        self.send_mtk_cmd (pmtk)
        rcv = self.get_mtk_status ()
        return rcv
    ...
    def get_fix_ctl(self):
        pmtk = "PMTK400"
        self.send_mtk_cmd (pmtk)
        rcv = self.get_mtk_status ()
        return rcv

    def get_dgps_mode(self):
        pmtk = "PMTK401"
        self.send_mtk_cmd (pmtk)
        rcv = self.get_mtk_status ()
        return rcv
    ...
    def read_rmc_non_blocking(self,timeout=0.01):
        msg=False
        val=[0.,'N',0.,'W',0.,0.0,0.0]
        # get RMC message with lat,lon,sog and cog
        ...
        try:
            ...
            print ("error read GPS RMC")
            return msg,val
        """
        v=self.ser.readline()
        # val = [lat,N/S,lon,E/W,time,sog,cog]
        if len(v)>0:
            st=v.decode("utf-8")
            if str(st[0:6]) == "$GPRMC":
                vv = st.split(",")
    ...
    def read_non_blocking(self,timeout=0.01,debug=False):
        msg=False
        val=""
        self.ser.timeout=timeout
        v=""
        v=self.ser.readline()
        # get all messages, no check on particular datagram
        if len(v)>0:
            val=v.decode("utf-8")
            msg=True
            if debug:
...
if __name__ == "__main__":
    ...
    # test non blocking read for 20 positions (RMC message)
    cnt=0
    while True:
    ...
    # test non blocking read for 20 positions (GLL message)
    cnt=0
    while True:
