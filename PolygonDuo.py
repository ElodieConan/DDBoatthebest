...
from communication_Thibault import *
...
communicant = Communication(port=29200,mission_start_time=0,mission_duration=T)
...
def controller_vit(PHI_obj, PHI):
    ...
    u2p = k_vit_p*np.tanh((PHI_obj - PHI)/k_vit_p)
    u2m = k_vit_m*np.tanh((PHI_obj - PHI)/k_vit_m)
    u2 =  min(u2p, u2m)
...
def SuiviDeLigne(lat_a, lon_a, lat_b, lon_b, i, label):
    ...
    while validation > 100:
        ...
        x_obj, y_obj, PHI_obj = communicant.data_front_boat
